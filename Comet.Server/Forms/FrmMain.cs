...
namespace Comet.Server.Forms
{
    ...
    {
        public CometServer ListenServer { get; set; }
            ...
            try
            {
                this.Invoke((MethodInvoker)delegate
                {
                    ...
                    this.Text = (selected > 0)
                        ? string.Format("Comet - Connected: {0} [Selected: {1}]", ListenServer.ConnectedClients.Length,
                            selected)
                        : string.Format("Comet - Connected: {0}", ListenServer.ConnectedClients.Length);
                });
            }
            catch (Exception)
            {
        ...
        private void InitializeServer()
        {
            ...
            ListenServer = new CometServer(serverCertificate);
        ...
        void GetWebcams()
        {
            foreach (Client c in GetSelectedClients())
            {
                FrmRemoteWebcam frmRw = FrmRemoteWebcam.CreateNewOrGetExisting(c);
                frmRw.Show();
                frmRw.Focus();
            }
        }
